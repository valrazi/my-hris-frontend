...
export const AdminAttendance: React.FC = () => {
  ...
  const columns: Column<AttendanceRecord>[] = [
    ...
    {
      header: 'Date',
      accessor: (item) => <span className="font-semibold text-slate-800">{dayjs(item.date).format('MMM D, YYYY')}</span>,
    },
    {
      header: 'Clock In',
      accessor: (item) => (
          ...
          {item.photoUrl ? (
            ...
            >
              <img
                ...
                className="w-8 h-8 rounded-lg object-cover border border-slate-200 group-hover:opacity-80 transition-opacity"
              />
              <span className="text-xs text-blue-600 group-hover:underline font-medium hidden sm:inline">
                Inspect Proof
              </span>
            </button>
          ) : (
  ...
  return (
    <div className="space-y-6">
      {/* Page Title */}
      <div>
        <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">HR Attendance Monitoring</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-xl font-bold text-slate-900 mt-0.5">{summary?.totalEmployees || 0}</div>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center gap-4">
          ...
          <div>
            <div className="text-xs text-slate-500 font-medium">Clocked In Today</div>
            <div className="text-xl font-bold text-slate-900 mt-0.5">{summary?.presentCount || 0}</div>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center gap-4">
          ...
          <div>
            <div className="text-xs text-slate-500 font-medium">Late Today</div>
            <div className="text-xl font-bold text-slate-900 mt-0.5">{summary?.lateCount || 0}</div>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center gap-4">
          ...
          <div>
            <div className="text-xs text-slate-500 font-medium">Not Clocked In</div>
            <div className="text-xl font-bold text-slate-900 mt-0.5">{summary?.notClockedInCount || 0}</div>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-xs bg-slate-50 p-4 rounded-xl border border-slate-200/60">
              <div>
                ...
                <span className="font-bold text-slate-900">{selectedRecord.employee?.firstName} {selectedRecord.employee?.lastName}</span>
              </div>
              <div>
                <span className="text-slate-400 block mb-0.5">NIK / Department:</span>
                <span className="font-bold text-slate-900">{selectedRecord.employee?.nik || 'N/A'} • {selectedRecord.employee?.department || 'N/A'}</span>
              </div>
              <div>
                <span className="text-slate-400 block mb-0.5">Clock In Timestamp:</span>
                <span className="font-bold text-slate-900">{dayjs(selectedRecord.clockInTime).format('YYYY-MM-DD HH:mm:ss')}</span>
              </div>
              <div>
                <span className="text-slate-400 block mb-0.5">Clock Out Timestamp:</span>
                <span className="font-bold text-slate-900">{selectedRecord.clockOutTime ? dayjs(selectedRecord.clockOutTime).format('YYYY-MM-DD HH:mm:ss') : 'Not Clocked Out Yet'}</span>
              </div>
            </div>

            {selectedRecord.workNotes && (
              <div className="text-xs bg-blue-50/50 p-3.5 rounded-xl border border-blue-100 flex items-start gap-2">
                <FileText className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" />
                <div>
                  <span className="font-semibold text-blue-900 block mb-1">Submitted Work Plan / Notes:</span>
